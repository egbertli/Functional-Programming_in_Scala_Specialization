...
object StructuredStreaming {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      ...
      .master("local[*]")
      .config("spark.sql.streaming.checkpointLocation", "/Users/yixuan-li/Documents/github/Functional-Programming_in_Scala_Specialization/Spark-Streaming/ApacheAccessLog/checkpoint")
      .getOrCreate()

    setupLogging()

    // Stream in structured JSON log data, count up "action" values for each hour, and write the results to a database
    // val inputDF = spark.readStream.json("s3://logs")
    // val inputDF.groupBy($"action", window($"time", "1 hour")).count().writeStream.format("jdbc").start("jdbc:mysql//...")


    // Create a stream of text files dumped into the logs directory
    val rawData = spark.readStream.text("/Users/yixuan-li/Documents/github/Functional-Programming_in_Scala_Specialization/Spark-Streaming/ApacheAccessLog")
